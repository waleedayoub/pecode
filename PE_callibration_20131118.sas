...
%LET PE_START_DATE = 20110101;
%LET j=2;
%LET CURRENT_PERIOD = 201303;
%LET PREVIOUS_PERIOD = 201203;
...
/* here you need to create a master table with PROMO_FLAG corresponding to the week
irrespective of the current week you're doing this for 
	- possible to append rather than recreate for future report iterations? */
proc sql;
create table promo_sku_fact as
        select b.SKU
                  ...
                  ,e.PROMO_PERIOD
                  ,e.NSLC_WEEK
				  ,case when b.SKU inner join (&PROMO_SKUS) then 1 else 0 end as PROMO_FLAG
...
%LET WEEKS = 8; /* set the moving average amount */
